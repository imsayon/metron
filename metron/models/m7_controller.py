...
def choose_rung(status: ModalityStatus) -> ControllerDecision:
    ...
    radar = (
        status.radar_valid and status.radar_age_min <= 20 and status.radar_coverage_fraction >= 0.30
    )
    lightning = (
        status.lightning_valid and status.lightning_age_min <= 10 and status.lightning_health > 0
    )
    nwp = status.nwp_valid
    if status.ood:
        return ControllerDecision(
            "R4", True, (Abstention("M7", "ood", "input is outside training support"),)
        )
    if not status.contract_ok:
        return ControllerDecision(
            "R4", False, (Abstention("M7", "contract_violation", "feature contract mismatch"),)
        )
    ...
    if sat and lightning and nwp:
        return ControllerDecision(
            ...
            False,
            (
                Abstention("M4", "no_radar", "hail module disabled"),
                Abstention("M5", "no_velocity", "velocity not guaranteed"),
            ),
        )
    if sat and nwp:
        return ControllerDecision(
            ...
            (
                Abstention("M4", "no_radar", "hail module disabled"),
                Abstention("M5", "no_velocity", "velocity not available"),
            ),
        )
    if nwp:
        return ControllerDecision(
            "R3", False, (Abstention("M1", "sat_missing", "no observation-driven arrival"),)
        )
